...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((command) =>
      command.setName(`source-code`).setDescription(`Want to buy the source code of this bot?`)
    )
    .addSubcommand((command) =>
      command
        .setName(`uptime`)
  ...
  async execute(interaction, client) {
    ...
    switch (sub) {
      case "support":
        await interaction.reply({ content: `https://discord.gg/YSv9VqQg6g` });
    }
    // Support
    switch (sub) {
      case "source-code":
        await interaction.reply({ content: `[Source Code Available on BuiltByBit](https://builtbybit.com/resources/razor-an-all-in-one-discord-bot.29648/)` });
    }
    ...
    switch (sub) {
      case "ping":
        ...
        const embed = new EmbedBuilder()
          .setTitle("**PONG!**")
          .setDescription(
            `<:ping:1271441984296587295> | **Latency:** \`${client.ws.ping}ms\`\n<:thunderr:1271447633789063262> | **Database Latency:** \`${dbPing}ms\``
          )
        ...
        collector.on("collect", async (i) => {
          if (i.customId == "btn") {
            i.update({
              content: `<:loading:1271447615736643637> Refreshed The Ping Stats`,
              embeds: [
                new EmbedBuilder()
                  .setTitle("**PONG!**")
                  .setDescription(
      ...
      case "changelogs":
        changelogs
          ...
          .exec(async (err, data) => {
            ...
            if (!data)
              interaction.reply({
                content: `> <:cross:1271441946283610195> No changelogs have been published`,
                ephemeral: true,
              });
            if (data) {
              const embed = new EmbedBuilder()
                ...
                .setDescription(
                  !data.config.description
                    ? "<:error:1271441954399453265> A new changelogs is here!"
                    : data.config.description
                )
                .setFooter({
                  text: `${

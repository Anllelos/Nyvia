...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    const booster = member.premiumSince
      ? `<a:boost:1096796252605255701> Yes`
      : `No`; //Change the emoji
    const ownerE = `<:owner:1271444966362714112>`; // change the server owner emoji
    const devs = `<:dev:1271441948741468216>`; // change the bot dev emoji
    const owners = [
      `536944480469909569`,
      `1192884985703702609`, // id for the devs of the bot
    ];
    const MutualServers = [];
    const JoinPosition = await interaction.guild.members.fetch().then(
      (Members) =>

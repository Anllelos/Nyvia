...
module.exports = {
    ...
    async execute (interaction, client) {
        ...
        switch (sub) {
            ...
            if (role.position >= interaction.guild.members.resolve(client.user).roles.highest.position) return await interaction.reply({ content: "The role you mentioned is too high! Place the bot's role above that role.", ephemeral: true })
            if (role2&&role2.position >= interaction.guild.members.resolve(client.user).roles.highest.position) return await interaction.reply({ content: "The role you mentioned is too high! Place the bot's role above that role.", ephemeral: true })
            if (role3&&role3.position >= interaction.guild.members.resolve(client.user).roles.highest.position) return await interaction.reply({ content: "The role you mentioned is too high! Place the bot's role above that role.", ephemeral: true })
            if (role4&&role4.position >= interaction.guild.members.resolve(client.user).roles.highest.position) return await interaction.reply({ content: "The role you mentioned is too high! Place the bot's role above that role.", ephemeral: true })
            if (role5&&role5.position >= interaction.guild.members.resolve(client.user).roles.highest.position) return await interaction.reply({ content: "The role you mentioned is too high! Place the bot's role above that role.", ephemeral: true })
            ...
            if (role2 !== null && level2 === null) return await interaction.editReply({ content: ":warning: Both fields must be filled out (role & level)", ephemeral: true })
            if (role3 !== null && level3 === null) return await interaction.editReply({ content: ":warning: Both fields must be filled out (role & level)", ephemeral: true })
            if (role4 !== null && level4 === null) return await interaction.editReply({ content: ":warning: Both fields must be filled out (role & level)", ephemeral: true })
            if (role5 !== null && level5 === null) return await interaction.editReply({ content: ":warning: Both fields must be filled out (role & level)", ephemeral: true })
    
            if (level2 !== null && role2 === null) return await interaction.editReply({ content: ":warning: Both fields must be filled out (role & level)", ephemeral: true })
            if (level3 !== null && role3 === null) return await interaction.editReply({ content: ":warning: Both fields must be filled out (role & level)", ephemeral: true })
            if (level4 !== null && role4 === null) return await interaction.editReply({ content: ":warning: Both fields must be filled out (role & level)", ephemeral: true })
            if (level5 !== null && role5 === null) return await interaction.editReply({ content: ":warning: Both fields must be filled out (role & level)", ephemeral: true })
            ...
            /*
                You can add a limit to how many roles they can add at one time
                const limit = 5;
                if (levelRole.Data.length >= limit) return await interaction.reply({ content: "You have the maximum amount of level roles! (5)", ephemeral: true })
            */
            ...
            if (!levelRole2) return await interaction.reply({ content: `:no_entry_sign: You currently do not have any level role system data setup! Do /level-roles set to include some data.`, ephemeral: true})
            else {
            ...
            if (!levelRole3) return await interaction.editReply({ content: `:no_entry_sign: You currently do not have any level role system data setup! Do /level-roles set to include some data.`, ephemeral: true})
            else {
                const embed = new EmbedBuilder()
                .setColor(client.config.embed)
                .setTitle(`Level Role Manager`)
